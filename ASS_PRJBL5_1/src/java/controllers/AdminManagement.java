...
public class AdminManagement extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("mode") != null && request.getParameter("mode").equals("adminupdate")) {

            String id = request.getParameter("id");
            user u = da.getUserId(id);
            request.setAttribute("u", u);
            request.getRequestDispatcher("UpdateAdmin.jsp").forward(request, response);
        }
        ArrayList<user> data = da.getUser();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (submit != null && submit.equals("BACK TO LOGIN")) {
            response.sendRedirect("Login.jsp");
        } else {
            userdao.UpdateAccount(role, id);
